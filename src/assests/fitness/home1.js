...
class HelloWorldApp extends Component {
    ...
    Item=(item)=> {
      ...
      // console.log("ICON",item.text)
        return (
            <View style={[styles.inner,{backgroundColor:item.color}]}>
            ...
            style={{height:200,width:200,marginLeft:20,position:'absolute'}}></Image>
                ...
                <Text style={{fontWeight:'bold',fontSize:20,left:60,color:"white"}}>{item.text}</Text>
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView horizontal={true} style={styles.botton}>
             <FlatList
               data={data}
               renderItem={({ item }) => this.Item1(item)}
               keyExtractor={item => item.id}
                horizontal={true}
                /> 
            <ImageBackground 
